...
/* eslint-disable @next/next/no-img-element -- The official logo is a pre-optimized static asset. */
  ...
  return (
    <header className={`site-header ${scrolled ? "is-scrolled" : "is-top"}`}>
      <div className="container header-inner">
        <Link className="site-logo" href="/#inicio" aria-label="Orion — início">
          <img
        ...
        >
          {links.map(({ label, href, match }) => (
            <Link key={href} href={href} aria-current={match(pathname) ? "page" : undefined} onClick={() => setOpen(false)}>{label}</Link>
          ))}
          <Link className="header-cta" href="/#contato" onClick={() => setOpen(false)}>Contato</Link>
        </nav>
      </div>
    </header>
  );
