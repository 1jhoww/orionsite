/* eslint-disable @next/next/no-img-element -- Official brand imagery is local, sized and loaded explicitly. */
      ...
      <div className="container home-portfolio-grid">
        {previewBrands.map((brand, index) => (
          <Reveal
            ...
            key={brand.name}
          >
            <a href={brandLinks[brand.name]} aria-label={`Explorar ${brand.name}`}>
              {index === 0 && (
